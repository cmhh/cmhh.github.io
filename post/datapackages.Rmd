...
An R package is essentially an eponymously named folder which exists somewhere
in your 'library path', and whose contents conform to certain conventions.  The
package source itself also resides in an eponymously named folder, but not
in your library path, and with slight differences in layout.

For example, a relatively minimal source package might contain the following:

| path        | type   | purpose                                                              |
|-------------|--------|----------------------------------------------------------------------|
| data        | folder | Contains datasets in R's native format (.rda).                       |
| man         | folder | Contains help files in R's native documentation markup format (.Rd). |
| R           | folder | Contains R source files.                                             |
| DESCRIPTION | file   | Package metadata including name, title, version, author, etc.        |
| NAMESPACE   | file   | Controls exports, dependencies, etc.                                 |

Traditionally, a loadable version of a package would be built from this source 
repository by running a command at a shell such as:

```
R CMD INSTALL --build pkgname
```

to both install the package and make a zip file for distribution for Windows 
platforms, or:
...
for every other platform.  After installing, there will be a folder containing
the package added to one of the folders in the library path, with some subtle 
difference in layout compared to the original source.  For example, help files 
will be processed to produce formatted HTML output.  
...
![Project->New Project...](/img/datapackages/package01.png)

![Create Project](/img/datapackages/package02.png)

![Project Type](/img/datapackages/package03.png)

![Create R Package](/img/datapackages/package04.png)

When working on a package, RStudio provides automation via a 'Build' menu.
From there you can build the package and create source or binary packages for
dissemination.

![Build menu](/img/datapackages/package05.png)
...
![Version control](/img/datapackages/versioning.png)


# Using R Packages to Distribute Data

As noted above, R pacakges can include datasets, and it is very common to do so.
This shouldn't really come as a surprise--R is a statistical language in the 
end, and it is normal to include working examples with pacakges; and including
datasets against which to run unit tests and other examples is clearly a 
useful thing to do.
...
purposes.  But this doesn't have to be the case.
...
this data via an R package (or set of packages).
...
![Shiny web app](/img/datapackages/shiny01.png)

![New Shiny Web Application](/img/datapackages/shiny02.png)

![Shiny application running within RStudio IDE](/img/datapackages/shiny03.png)
...
highest level of each variable’s classification.
...
![Help for `dwelling` dataset.](/img/datapackages/dwellinghelp.png)
...
![`help(package="NZCensus")`.](/img/datapackages/NZCensushelp.png)

![`NZCensus` docs.](/img/datapackages/NZCensusdoc.png)
...
classes aren't terribly terribly large (we haven't included, amusingly, the
meshblock feature class, which is pretty large), but at least this way users who
only want the Census datasets aren't burdened with the extra bulk implied by the
demo package if they don't want it.  Similarly, the demos have dependencies
which would need to be installed along with them, and loaded any time the demo
package is.  So, again, if users have no need for the demos, then they wouldn't
be forced to install these additional dependencies if they didn't want them.
...
![`NZCensusDemos::mapdemo()`](/img/datapackages/NZCensusMapDemo.png)
...
![```NZCensusDemos::distributiondemo()```](/img/datapackages/NZCensusDistributionDemo.png)
...
![lots of duplication!](/img/datapackages/rawlabmkt.png)
...
![metadata](/img/datapackages/metadata.png)

![metadata](/img/datapackages/labmkt.png)
...
![```timeseriesdemo()```](/img/datapackages/SNZlabmktTSDemo.png)
...
![Git](/img/datapackages/git01.png)

![Git](/img/datapackages/git02.png)

![Git diff](/img/datapackages/git03.png)
