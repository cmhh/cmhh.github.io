...
The ```fromJSON``` function turns a JSON object to an R list.  If geocoding is 
successful (and we will just assume it was) then the origin and
destination objects will contain a number of elements.  We're particularly 
interested in latitude, longitude, and the formatted address.  E.g.

```{r p1}
origin$results[[1]]$geometry$location$lat
origin$results[[1]]$geometry$location$lng
